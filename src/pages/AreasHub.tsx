...
const AreasHub = () => (
  <Layout>
    <SEOHead
      title="Areas Served | Professional Painters Merrimack Valley MA"
      description="We serve homeowners across the Merrimack Valley including Andover, Haverhill, Lowell, Methuen, Billerica, and surrounding towns."
      canonical="https://example.com/areas"
    />
      ...
      <div className="container-site grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {cities.map((c, i) => (
          <FadeIn key={c.slug} delay={i * 0.04}>
            <Link to={`/${c.slug}`} className="card-elevated p-8 block group">
              <h2 className="text-display text-xl mb-2 group-hover:text-accent transition-colors">
                {c.name}, {c.state}
              </h2>
              <p className="text-muted-foreground text-sm leading-relaxed mb-4">{c.contextualCopy}</p>
